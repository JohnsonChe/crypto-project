...
function WatchList(props) {
    ...
    try {
      if (watch) {
        ...
        for (let i = 0; i < Math.min(pageItems, watch.length); i++) {
          listItemArr.push(<ListItem key={i} crypto={watch[counter]} fav={true} buttonClick={handleRemoveButton} />);
          counter++;
        }
  ...
  async function fetchWatchList(watchListing) {
    ...
    const data = await Promise.all(watchListing.map(element => _fetch(element)));
  ...
  const handleRemoveButton = async (data) => {
    ...
    updatedWatchListData = updatedWatchListData.filter(element => element.id !== data.id)
